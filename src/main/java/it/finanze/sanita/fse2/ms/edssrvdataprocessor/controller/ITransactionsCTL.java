...
import static it.finanze.sanita.fse2.ms.edssrvdataprocessor.config.Constants.Logs.ERR_VAL_FUTURE_DATE;
import static it.finanze.sanita.fse2.ms.edssrvdataprocessor.config.Constants.Logs.ERR_VAL_PAGE_IDX_LESS_ZERO;
import static it.finanze.sanita.fse2.ms.edssrvdataprocessor.config.Constants.Logs.ERR_VAL_PAGE_LIMIT_LESS_ZERO;
...
import static it.finanze.sanita.fse2.ms.edssrvdataprocessor.utility.RoutesUtility.API_TRANSACTIONS_TAG;
...
public interface ITransactionsCTL {
    ...
    GetTxPageResDTO getTransactions(
            @RequestParam(API_QP_TIMESTAMP) @Parameter(description = "Identificatore arco-temporale") @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) @NoFutureDate(message = ERR_VAL_FUTURE_DATE) Date timestamp,
            @RequestParam(API_QP_PAGE) @Parameter(description = "Indice pagina richiesto (eg. 0, 1, 2...)") @PositiveOrZero(message = ERR_VAL_PAGE_IDX_LESS_ZERO) int page,
            @RequestParam(API_QP_LIMIT) @Parameter(description = "Limite documenti per pagina (eg. 10, 20 ...)") @Positive(message = ERR_VAL_PAGE_LIMIT_LESS_ZERO) int limit)
